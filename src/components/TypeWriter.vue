<template>
    <span class="type-writer" :class="{'type-writer-current': active }" v-html="text"/>
</template>
        ...
        methods: {
            ...
            type() {
                ...
                if (this.lettersToWrap.indexOf(this.wordIndex) !== -1) {
                    this.text += `<span class="type-writer-first-letter">${this.word[this.wordIndex]}</span>`;
                } else {
...
<style scoped lang="scss">
    .type-writer-current {
        overflow: hidden;
        border-right: .15em solid $mainColor;
        white-space: nowrap;
        margin: 0 auto;
        letter-spacing: .10em;
